...
#write.csv(ar_west, "arswestcoast.csv", row.names=FALSE) #create smaller file to work with
westcst_ar <- read.csv('Data/arswestcoast.csv', header=TRUE)
...
floods$nFlood_date <- with(floods, ymd(sprintf('%04d%02d%02d', Year, Month, Day))) #correct format for flood only dates
